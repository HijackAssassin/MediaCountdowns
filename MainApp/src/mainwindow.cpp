...
void MainWindow::repositionPicker()
{
    ...
    static constexpr int ROW_H = 54, VISIBLE = 3, DISMISS_H = 28;
    int h      = ROW_H * VISIBLE + DISMISS_H + 2;
    int margin = 6;
    int w      = centralWidget()->width() - margin * 2;
    int y      = m_bottomBar->geometry().top() - h - 4;
    m_pickerFrame->setGeometry(margin, y, w, h);
    m_pickerList->setFixedHeight(ROW_H * VISIBLE);
}
...
void MainWindow::rebuildTab(int tab)
{
    ...
    std::sort(inTab.begin(), inTab.end(), [tab](TileWidget* a, TileWidget* b){
        if (tab == TAB_RELEASED)
...
void MainWindow::onScraperDataReady(const TileData& data)
{
    ...
    m_searchEdit->clear();
    m_tiles.append(data);
    appendTileWidget(data);
    saveTiles();
    m_statusLbl->setStyleSheet("color:#66bb66; font-size:11px;");
    m_statusLbl->setText(QString("Added: %1").arg(data.title));
}
    ...
    for (TileWidget* tw : std::as_const(m_tileWidgets)) {
        ...
        for (TileData& td : m_tiles) {
            ...
            if (oldTab == TAB_RELEASED && newTab == TAB_ACTIVE) {
                ...
                tw->updateData(td);   // sync widget's copy too
                APPLOG(QString("onTileDataChanged: '%1' released→active, notifStatus reset to Active").arg(td.displayTitle()));
            }
...
void MainWindow::onGlobalTick()
{
    ...
    for (TileWidget* tw : std::as_const(m_tileWidgets)) {
        ...
        if (td.notifStatus == NotifStatus::Active && td.hasDate() && td.isExpired()) {
            APPLOG(QString("onGlobalTick: tile '%1' just expired — moving to Released tab").arg(td.displayTitle()));
            for (TileData& mtd : m_tiles) {
                if (mtd.id != td.id) continue;
                mtd.notifStatus = NotifStatus::Ready;
                // Sync widget's data copy so tabForTile() sees the updated state
                tw->updateData(mtd);
                anyExpired = true;
                // Fire the notification directly from the main app — same path
                // as the test notification so it is guaranteed to work.
                APPLOG(QString("onGlobalTick: firing notification for '%1'").arg(mtd.displayTitle()));
