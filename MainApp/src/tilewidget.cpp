...
//  tabVisible = true  → tile's tab is shown → decrement and repaint.
//  tabVisible = false → skip; no GPU work for hidden tabs.
// =============================================================================
void TileWidget::tick(bool tabVisible)
{
    ...
    // Just decrement — no datetime construction, no diff calculation
    if (m_remainingSecs > 0) m_remainingSecs--;
...
void TileWidget::contextMenuEvent(QContextMenuEvent* event)
{
    ...
    // menu.exec() already returned, but we're still inside Qt's event dispatch
    // for this widget. A synchronous emit → onRemoveTile → delete this would
    // crash as Qt tries to unwind through a destroyed widget. Queue it instead.
    if (chosen == removeAct) {
...
void TileWidget::openEditDialog()
{
    ...
    bool dateChanged = (newDate != m_data.customDate) ||
                       (!newDate.isValid() && m_data.effectiveDate().isValid() && m_data.targetDate.isValid());
    if (dateChanged) {
