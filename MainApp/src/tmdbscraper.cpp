...
void TmdbScraper::searchMedia(const QString& query)
{
    ...
    auto processPage = [this, state, q, yearFilter](const QJsonArray& arr) {
        ...
        for (const QJsonValue& v : state->combined) {
            ...
            // ── First-word anchor — skip leading articles then compare ─────────
            // "Avengers" must not match "Lego Avengers": the first meaningful
            // word of the title must equal the first word of the query.
            if (!queryWords.isEmpty()) {
                static const QSet<QString> articles = {"the","a","an"};
                QString firstTitle;
                for (const QString& w : titleWords)
                    if (!articles.contains(w)) { firstTitle = w; break; }
                if (firstTitle != queryWords[0]) continue;
            }
